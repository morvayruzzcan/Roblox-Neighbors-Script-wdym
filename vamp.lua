...
-- ─────────────────────────────────────────────────────────────
-- METAMETHOD HOOKS (wrapped in pcall for Xeno compatibility)
-- If hookmetamethod is unavailable, UI still loads normally
-- ─────────────────────────────────────────────────────────────
local oldIndex
pcall(function()
...
-- detachWingsFromBack MUST be defined before create3DWings (forward ref fix)
...
-- Detect if the currently held tool is a wing-type item (MUST be before startFly)
...
-- Safe GUI cleanup (works on Xeno, Solara, Volt — no CoreGui crash)
pcall(function()
    local targets = {}
    pcall(function()
        local old = CoreGui:FindFirstChild("WDYM_VAMP_UI")
        if old then table.insert(targets, old) end
    end)
    pcall(function()
        local pg = lp:FindFirstChild("PlayerGui")
        if pg then
            local old = pg:FindFirstChild("WDYM_VAMP_UI")
            if old then table.insert(targets, old) end
        end
    end)
...
-- =====================================================
-- UNIVERSAL GUI PARENTING
-- Xeno and Solara block CoreGui — ALWAYS try PlayerGui
-- first, then gethui(), then CoreGui as last resort.
-- =====================================================
local _guiParented = false

-- Step 1: Try PlayerGui FIRST (works on ALL executors 100%)
if not _guiParented then
    pcall(function()
        local pg = lp:WaitForChild("PlayerGui", 3)
        if pg then
            GUI.Parent = pg
            _guiParented = (GUI.Parent ~= nil)
        end
    end)
end

-- Step 2: Try gethui() (Synapse X, Wave, some Xeno builds)
if not _guiParented then
    pcall(function()
...
-- Step 3: CoreGui fallback (Volt, high-level executors)
if not _guiParented then
    pcall(function()
...
pcall(function()
    StarterGui:SetCore("SendNotification", {
        Title = "WDYM Hub",
        Text = "Script loaded successfully! Press Left/Right Alt to toggle UI.",
        Duration = 5
    })
end)
